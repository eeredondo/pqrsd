...
def reasignar_solicitud(
    ...
    nuevo_responsable = db.query(Usuario).filter(Usuario.id == datos.nuevo_responsable_id).first()
    if not nuevo_responsable:
        raise HTTPException(status_code=404, detail="Usuario responsable no encontrado")

    solicitud.asignado_a = nuevo_responsable.id
    db.commit()

    evento = Trazabilidad(
        solicitud_id=solicitud.id,
        evento="Reasignación",
        mensaje=f"Solicitud reasignada a {nuevo_responsable.nombre}.",
        usuario_remitente=usuario_obj.nombre,
        usuario_destinatario=nuevo_responsable.nombre
    )
    db.add(evento)
    db.commit()

    return {"mensaje": "Solicitud reasignada correctamente"}
